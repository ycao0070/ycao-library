...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(amount, storetype) in bookstores.storeTypes" :key="storetype">
          {{ storetype }} {{ amount }}
        </li>
      </ul>
      ...
      <ul>
        <li v-for="(time, days) in bookstores.openingHours" :key="days">
      ...
      <p>Our #1 seller (separatly):</p>
      <ul>
        <li v-for="top in bookstores.topSellers" :key="top">
          {{ top }}
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <p>
        ...
        <span :style="{ color: textColor, fontSize: fontSize + 'px' }">{{ orwell.name }}</span>
      </p>
    </section>
  </div>
</template>
...
const austen = computed(() =>
  ...
  authors.find((austen) => (austen.id = 1))
)
